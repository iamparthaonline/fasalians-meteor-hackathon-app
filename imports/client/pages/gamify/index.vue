<template>
  <div class="gamify-page">
    <TopNavBar :is-back-button="true" title="Level Details" />
    <main>
      ...
      <div class="text-center">
        <v-btn small @click="sheet = !sheet">How to get points</v-btn>
      </div>
      <br />
        ...
        <div class="py-3 text-left">
          <ul class="point-tactics">
            <li v-for="{label, point, max, key} in pointMakingTactics" :key="key">
              <span class="d-flex flex-no-wrap justify-space-between point-name">
                <span>{{ label }}</span>
                <span class="point">{{ point }} points</span>
              </span>
      ...
      <ul class="point-tactics" v-if="pointMakingTactics.length > 0">
        <li v-for="{type, createdAt} in pointsHistory" :key="type">
          <span class="d-flex flex-no-wrap justify-space-between point-name">
            <span>{{ pointMakingTactics.find(({key}) => key === type).label }}</span>
            <span class="point">
              {{ pointMakingTactics.find(({key}) => key === type).point }} point{{
                pointMakingTactics.find(({key}) => key === type).point > 1 ? 's' : ''
              }}
            </span>
          </span>
